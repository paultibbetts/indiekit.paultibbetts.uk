...
/** @type {{accessKey: string, secretKey: string, region: string, endpoint: string, bucket: string, publicUrl: string, acl: ObjectCannedACL | false, forcePathStyle: boolean}} */
const defaults = {
...
export default class S3Store {
   ...
   * @param {string} [options.publicUrl] - Public URL for uploaded files
   * @param {ObjectCannedACL | false} [options.acl] - Access Control List (ACL) policy
   * @param {boolean} [options.forcePathStyle] - Force path-style S3 URLs
   */
  constructor(options = {}) {
  ...
  get publicUrl() {
    const { bucket, endpoint, publicUrl } = this.options;

    return publicUrl || `${endpoint}/${bucket}`;
  }
  ...
  async createFile(filePath, content) {
    ...
    if (this.options.acl) {
      putCommandInput.ACL = this.options.acl;
    }
  ...
  async updateFile(filePath, content, options) {
    ...
    try {
      ...
      if (ETag) {
        ...
        return this.fileUrl(updatedFilePath);
      }
    } catch (error) {
      throw new IndiekitError(error.message, {
        cause: error,
